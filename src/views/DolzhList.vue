<template>
	<div class="panel panel-primary" style="margin-top: 10px">
		<div class="panel-heading" id="hd">Должности</div>
		<div class="panel-body">
			<router-link role="button" class="btn btn-lg btn-block btn-default" :to="{name: 'editDolzh', params: {id: 0}}">
				Добавить
			</router-link>
		...
		<table class="table table-striped">
			...
			<tbody>
			<tr class="st" v-for="(item, id) in dolzh" :key="id">
				<td>{{id + 1}}</td>
				<td>{{ item.dolzh_name }}</td>
				<td><router-link role="button" class="btn btn-sm btn-link" :to="{name: 'editDolzh', params: {id: item.id}}">
					<span class="glyphicon glyphicon-pencil"></span>
				</router-link></td>
				<td><button type="button" class="btn btn-sm btn-link" @click="handleRemoveClick($event, item.id)">
					<span class="glyphicon glyphicon-remove"></span>
